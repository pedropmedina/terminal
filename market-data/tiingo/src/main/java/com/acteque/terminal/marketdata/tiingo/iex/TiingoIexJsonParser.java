...
import static com.acteque.terminal.marketdata.tiingo.TiingoResponseDecoder.optionalInstant;
...
import static com.acteque.terminal.marketdata.tiingo.TiingoResponseDecoder.requiredInstant;
...
final class TiingoIexJsonParser {
  ...
  static List<IntradayData> parsePrices(String symbol, String json) {
    List<HistoricalPriceResponse> responses = readList(json, HISTORICAL_PRICES, "IEX historical prices");
    try {
      List<IntradayData> bars = new ArrayList<>(responses.size());
      for (HistoricalPriceResponse response : responses) {
        bars.add(response.toIntraday(symbol));
      }
      bars.sort(Comparator.comparing(IntradayData::timestamp));
  ...
  private record HistoricalPriceResponse(
    ...
    IntradayData toIntraday(String symbol) {
