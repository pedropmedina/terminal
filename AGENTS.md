...
- Order constructor parameters, record components, and method parameters by domain-first semantics: identity, domain configuration, domain data, supporting services, infrastructure concerns such as executors or clocks, and callbacks. Preserve that order in declarations, delegating calls, call sites, validation and initialization, and Javadoc `@param` tags. Prefer this semantic progression over alphabetical or type-based ordering. When language constraints or externally fixed interfaces require an exception, keep the local order consistent and document non-obvious choices.
- Document all Java classes, constructors, and methods with Javadoc, including package-private, protected, and private declarations. Explain purpose, behavior, and non-obvious contracts or side effects, especially for complex logic. Include `@param` for every parameter and `@return` for every non-void method, describing each value's meaning; keep documentation and tag order accurate as code changes.
- Organize classes for top-to-bottom reading: constants, fields, constructors, public queries, public commands, lifecycle methods, private event handlers, and utilities. Keep closely related methods together when that produces a clearer semantic flow.
- Prefer focused, behavior-preserving readability refactors. Separate input validation, state initialization, component composition, and event wiring into clear phases; extract descriptive helpers for distinct phases or duplicated behavior; remove fields needed only during construction; and avoid abstractions that merely rename straightforward one-line operations. Preserve public APIs unless a breaking change is explicitly requested.
...
- Structure UI feature packages under `app/src/main/java/` using MVCI: a public feature facade composes package-private model, view, and interactor types; the model owns observable feature state, the interactor owns state transitions and workflows, and the view observes the model and forwards user intents. Keep feature state and state transitions out of view builders and composition facades.
