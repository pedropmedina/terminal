...
- Document Java classes, constructors, and methods with Javadoc. Explain purpose, behavior, and non-obvious contracts or side effects, especially for complex logic. Include `@param` for every parameter and `@return` for every non-void method, describing each value's type and meaning; keep documentation accurate as code changes.
